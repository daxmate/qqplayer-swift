...
//  该侧已在 main 由 `SyncConnectDiagTests.swift`（7 例）覆盖，本文件不重复。
//
...
//  `SyncListener.init(localIdentity:trustStore:deviceName:)`、`NWPeerChannel(connection:queue:)`、
//  `SyncTCPParameters.make()`、`SyncConnectDiag.describe(_:)` 签名与输出格式逐一核对一致。
//
...
struct NWPeerChannelCloseTests {
    ...
    func closeNotifiesCancelledExactlyOnce() {
        ...
        channel.closeTransport()
        queue.sync {} // 排空通道串行队列：两次关闭都已执行完（有界等待，不用墙钟）
