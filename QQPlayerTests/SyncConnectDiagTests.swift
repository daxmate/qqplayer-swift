...
struct SyncConnectDiagTests {
    ...
    @Test("落点默认：未设 override 时落在 App 容器日志目录（iOS = Documents/.qqplayer/logs）")
    func defaultLogFileURLIsAppContainerDocuments() {
        ...
        // 2026-09-22 曲库文件夹化：日志经 `LibraryRoot.logsDirectoryURL()` 收进日志目录。
        // 断言走**同一个 scoped 入口**（iOS → `Documents/.qqplayer/logs`，macOS → `Documents/Logs`）——
        // 手拼「Documents + 目录名」在 iOS 会漏掉隐藏根（2026-09-22 CI 实证：实得
        // `Documents/.qqplayer/logs/sync-diag.log`）。
        #expect(resolved == LibraryRoot.logsDirectoryURL()?
            .appendingPathComponent(LibraryRoot.syncDiagnosticsLogFileName))
        // 非空转：日志目录必须真的解析得到（否则上面的相等退化成 `nil == nil`）。
        #expect(resolved != nil)
