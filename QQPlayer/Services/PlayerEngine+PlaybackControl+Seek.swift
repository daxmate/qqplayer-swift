...
//  2026-09-19 从 PlayerEngine+PlaybackControl.swift 原样搬出（纯搬家）。
    ...
    extension PlayerEngine {
        func seek(to time: TimeInterval) async {
            AppLog.info(.general, "⏪ seek(to: \(time)) called - usingSFBEngine: \(usingSFBEngine)")
            clearPreloadedNext()
            ...
            if usingSFBEngine {
                do {
                    ...
                    playbackTime = time
                    AppLog.info(.general, "✅ SFBAudioEngine seeked to: \(time)s")
                    ...
                    if nsError.domain == "SFBAudioEngineManager" && nsError.code == 4 {
                        ...
                        // 后续 play() 重载时会以该位置恢复。
                        AppLog.error(.general, "❌ Failed to seek with SFBAudioEngine: \(error) — falling back to native engine")
                        ...
                        AppLog.error(.general, "❌ SFBAudioEngine seek failed: \(error) — keeping position at \(playbackTime)s")
                        return
                    }
                }
            }
            ...
            guard let audioFile = audioFile,
                  !isLoadingTrack else {
                AppLog.warn(.general, "⚠️ Cannot seek: audioFile=\(audioFile != nil), loading=\(isLoadingTrack)")
                return
            }
            ...
            guard framePosition >= 0 && framePosition < audioFile.length else {
                AppLog.error(.general, "❌ Invalid seek position: \(framePosition), file length: \(audioFile.length)")
                return
            }

            AppLog.info(.general, "🔍 Seeking to: \(time)s (frame: \(framePosition))")
            ...
            if !audioEngine.isRunning {
                do {
                    ...
                    AppLog.info(.general, "✅ Started audio engine before scheduling (seek)")
                } catch {
                    AppLog.error(.general, "❌ Failed to start audio engine during seek: \(error)")
                    return
                }
            }
            ...
            AppLog.info(.general, "✅ Seek completed")
        }
    }
#endif
