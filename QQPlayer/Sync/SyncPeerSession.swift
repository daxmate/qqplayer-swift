...
final class SyncPeerSession: @unchecked Sendable {
    ...
    /// + manifest_request/manifest_response + sync_fetch_request/sync_fetch_result
    /// + library_push_announce + peer_library_request/peer_library_response；加密）。
    /// 未 ready / 类型非法抛错。
    /// ⚠️ 新增业务帧必须同时登记此处（漏登记 → 运行期 invalidType，编译不报错）。
